...
/* Token vindo do Kconfig; string vazia => auth desligada */
...
/* Função que valida o header Authorization */
static esp_err_t check_auth(httpd_req_t *req) {
    ...
    *end = '\0';
    ...
    /* Compara o token enviado com o configurado no Kconfig */
    const char *recv_token = start + plen;

    /* Trim final novamente por segurança (após remover prefixo) */
...
/* Função auxiliar para enviar resposta de erro em texto puro */
...
static esp_err_t ota_post(httpd_req_t *req) {
    if (check_auth(req) != ESP_OK) {
        /* Se falhar autenticação, retorna 401 sem usar httpd_resp_send_err */
    ...
    /* Buffer alocado no heap para chunks */
    ...
    /* Loop de recebimento e gravação da imagem */
    while (remaining > 0) {
        ...
        /* Log de progresso a cada ~128 KB */
    ...
    /* Define a partição como bootável */
    ...
    /* Responde OK e reinicia o dispositivo */
    httpd_resp_set_hdr(req, "Connection", "close");
