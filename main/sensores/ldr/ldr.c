#include "ldr.h"

static adc_oneshot_unit_handle_t adc_handle;
static adc_oneshot_chan_cfg_t chan_cfg = {
    .atten = LDR_ATTEN,
    .bitwidth = ADC_BITWIDTH_12
};

void ldr_init(void) {
    adc_oneshot_unit_init_cfg_t init_cfg = {
        .unit_id = LDR_ADC_UNIT
    };

    esp_err_t err;

    err = adc_oneshot_new_unit(&init_cfg, &adc_handle);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Falha ao inicializar ADC: %s", esp_err_to_name(err));
        return;
    }

    err = adc_oneshot_config_channel(adc_handle, LDR_ADC_CHANNEL, &chan_cfg);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Falha ao configurar canal do ADC: %s", esp_err_to_name(err));
    }
}

int ldr_get_luminosidade(void) {
    int raw = 0;
    esp_err_t err = adc_oneshot_read(adc_handle, LDR_ADC_CHANNEL, &raw);

    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Falha ao ler LDR: %s", esp_err_to_name(err));
        return -1;
    }

    // Converte para escala de 0 a 100 (quanto mais luz, maior o valor do LDR)
    int lum = (raw * 100) / 4095;
    if (lum < 0) lum = 0;
    if (lum > 100) lum = 100;
    return lum;
}
